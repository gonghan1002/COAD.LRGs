...
# read data
res.degs <- read.csv("./res/1.degs/table/degs.tumor_normal.fc1.csv")

# interest gene
interest_genes <- read.xlsx("./list/lactylation-related genes.xlsx")
colnames(interest_genes) <- "gene"
interest_genes$ont <- "Lactylation"
...
# save
outfile_tmp <- paste0(res_home,proj_name, "/table/","GSEA.", "Lactylation", ".csv");outfile_tmp
write.csv(kk.df,file = outfile_tmp)

# plot
outfile_tmp <- paste0(res_home,proj_name, "/pdf/","GSEA.", "Lactylation", ".pdf");outfile_tmp
